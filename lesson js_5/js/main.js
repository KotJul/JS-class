...
// средне арифмитическое всех элементов

let arithMean = 0;
console.log(s / arr.length);

// сумма всех элементов на четных позициях
...
// сделать все числа отрицательными

let r = arr.map(e => e * (-1));
console.log(r);

// сделать вот так ['Минск','Брест','Москва']
let arrr = ['миНск','брЕСТ','МоСкВа'];

let change = arrr.map ( (e) =>  e[0].toUpperCase() + e.slice(1).toLowerCase());/*e.slice(1).toLowerCase() - сначала e.slice(1) выделяет  все слово начиная с первой позиции(т.е со второй буквы), потом к тому что получилось применяет ловеркейс (делает все буквы маленькими)*/
console.log(change);